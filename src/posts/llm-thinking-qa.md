...
## 问题 12：多实例推理场景下，是否要关注用户会话保持？确保同用户推理请求到同一个推理实例

### 我的思考 / 问题

问题 1 已经说明推理后端是无状态的，每次请求自带完整 messages。但如果部署了多个 vLLM 实例（比如 2 台机器各跑一个 GLM-5.1），通过负载均衡分发请求，是否需要做**会话保持**（sticky session），让同一用户始终打到同一个实例？

### 答疑结论

**不需要为"正确性"做会话保持，但可以为"性能"做亲和路由。**

关键在于：LLM 推理是无状态的，每个请求自带完整上下文，打到任何一个实例都能得到完全正确的结果。会话保持不是功能需求，而是性能优化手段。

| 维度     | 是否需要同一实例 | 原因                                     |
| ------ | -------- | -------------------------------------- |
| 推理正确性  | **不需要**  | 每个请求独立，messages 自包含                    |
| 用户隔离   | **不需要**  | KV Cache 按请求隔离，不会串话                    |
| 前缀缓存命中 | **有帮助**  | 同一实例的 Prefix Cache 更可能命中，减少 TTFT       |
| 连续对话延迟 | **有帮助**  | Prompt Cache 命中时跳过已处理的 token，加速首 token |

### 扩展知识

#### 12.1 为什么正确性不依赖会话保持

问题 1 和问题 11 已经建立的核心事实：

```text
客户端发送的 messages 数组 = 完整对话历史
vLLM 收到后 → 从头计算 → 返回结果
任何实例都能独立完成这个过程
```

不存在"这个实例知道上一轮说了什么，那个实例不知道"的情况——因为每一轮请求里都带了所有历史消息。

#### 12.2 Prefix Cache 命中带来的性能收益

虽然正确性不需要会话保持，但**同一用户连续请求打到同一实例**时，vLLM 的 Prefix Cache 更可能命中：

```text
请求 1（用户 A）：[system, user1, assistant1, user2]     → 计算并缓存 [system] 的 KV
请求 2（用户 A）：[system, user1, assistant1, user2, assistant2, user3]
                    ↑ [system, user1, assistant1, user2] 已缓存 → 命中 → 跳过计算

若请求 2 打到另一个实例：缓存未命中 → 全量计算 → TTFT 更长
```

收益：

| 场景 | Prefix Cache 命中 | TTFT |
|------|-------------------|------|
| 同用户 → 同实例 | 概率高（system prompt + 历史可复用） | 更短 |
| 同用户 → 不同实例 | 概率低（仅 system prompt 可能全局共享） | 更长 |
...
```text
用户 → Nginx / HAProxy / 云 LB（轮询或最少连接）→ vLLM 实例 1
                                                   → vLLM 实例 2
                                                   → vLLM 实例 3
```

- 不做会话保持
- 负载均衡器按轮询（round-robin）或最少连接（least-connections）分发
- 简单可靠，适合请求量均匀的场景

**策略二：基于用户标识的亲和路由（性能优化）**

```text
用户 → 网关（hash(user_id) % N）→ 固定 vLLM 实例
```

- 对同一用户做 hash 路由，保证大部分请求打到同一实例
- Prefix Cache 命中率提升，TTFT 更稳定
- 需要网关层支持（LiteLLM、Kong、Envoy 等均可配置）

**策略三：vLLM Cache-Aware Routing**

vLLM 自身支持基于 KV Cache 状态的智能路由：

```text
Router → 查询各实例的 Prefix Cache 状态
       → 选择缓存命中率最高的实例分发请求
```

比固定 hash 更灵活：当某个实例缓存被驱逐或重启时，自动切换到其他有缓存的实例。

#### 12.4 会话保持的副作用

| 问题 | 说明 |
|------|------|
| 负载不均 | 某用户高频发请求时，目标实例过载，其他实例空闲 |
| 实例故障影响 | sticky 的实例挂了，该用户所有请求受影响 |
| 扩缩容困难 | 新增实例后需要重新 hash，部分用户的缓存会失效 |

#### 12.5 与问题 1、问题 11 的关系

- **问题 1**：推理后端无状态，每次请求独立
- **问题 11**：后端不管理用户上下文，用户识别在应用层
- **问题 12**：即使多实例部署，也不需要会话保持来保证正确性；会话保持是性能优化手段，且有副作用

```text
正确性 ← 由 messages 完整性保证（应用层责任）
性能   ← 可通过亲和路由优化 Prefix Cache 命中
负载   ← 会话保持可能引入不均衡，需权衡
```

**一句话总结：多实例推理场景下，会话保持不是正确性需求（每个请求自带完整上下文，打到任何实例结果都一样），而是 Prefix Cache 命中的性能优化手段。生产环境中，普通负载均衡即可满足大多数场景；对 TTFT 敏感时可考虑基于用户标识的亲和路由或 vLLM Cache-Aware Routing，但需注意负载不均和故障域的副作用。**
